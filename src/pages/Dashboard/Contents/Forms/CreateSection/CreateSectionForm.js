...
class CreateSectionForm extends Component {
                    ...
                    if (json.CODE === 1) {
                        if (json.DATA) {
                            if (json.DATA.length > 0) {
                                this.getProgrammeData(json.DATA[0].sessionId, json.DATA[0].programmeGroupId);
                                this.getCoursesData(json.DATA[0].programmeGroupId, json.DATA[0].courseId, json.DATA[0].sessionId);
                                this.setState({
    ...
    isFormValid = () => {
        let isValid = true;
        let { sessionIdError, programmeIdError, offeredCoursesIdError, sectionDataError, nameError } = this.state;
        ...
        this.setState({
            sessionIdError, programmeIdError, offeredCoursesIdError, sectionDataError, nameError
        })
    ...
    render() {
        ...
        const columns = [
            ...
            {
                name: "type", title: "Section Type", getCellValue: row => {
                    return (
                        <Fragment>{row.type === 1 ? 'Lecture' : 'Tutorial'}</Fragment>);
                }
            },
            {
                name: "action", title: "Action", getCellValue: row => {
                    return (
                        <Tooltip title="Delete">
                            <IconButton
                                onClick={() => this.deleteRecord(row.index)}
                            >
                                <DeleteIcon
                                    fontSize="small"
                                />
        ...
        return (
            <Fragment>
                    ...
                    >
                        ...
                        <Grid item xs={this.state.recordId == 0 ? 8 : 9}>
                            <TextField
                                ...
                                name="name"
                                disabled={!this.state.sessionId || !this.state.programmeId || !this.state.offeredCoursesId}
                                label={`${this.state.section === 1 ? 'Lecture' : 'Tutorial'} Section Name`}
                                required
                                fullWidth
                        ...
                        {this.state.recordId == 0 &&
                            <Grid item xs={1}>
                                <Button disabled={!this.state.sessionId || !this.state.programmeId || !this.state.offeredCoursesId} color="primary" style={{
                                    padding: 10,
                                    width: 85,
                                    marginTop: 5,
                                }} variant="outlined" onClick={() => this.onAddClick()}>
                                    <AddIcon /> Add
                                </Button>
                            </Grid>
                        }
                ...
                <form id="myForm" onSubmit={this.onFormSubmit}>
                    ...
                    <input type="hidden" name="programmeGroupId" value={this.state.programmeId} />
                    <input type="hidden" name="courseId" value={this.state.offeredCoursesId} />
                    {this.state.recordId != 0 ? <Fragment>
                        <input type="hidden" name="label" value={this.state.name} />
                        <input type="hidden" name="sectionTypeId" value={this.state.section} />
                    </Fragment> :

...
import F231FormFilter from "./F231FormFilter";
import F231FormTableComponent from "./F231FormTableComponent";
...
class F231Form extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  loadAcademicSession = async () => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C231CommonAcademicSessionsView`;
    await fetch(url, {
      method: "POST",
      headers: new Headers({
  ...
  loadProgrammeGroups = async (academicsSessionId) => {
    ...
    this.setState({ isLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C231CommonAcademicsSessionsOfferedProgrammesView`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
  ...
  loadCourses = async (academicsSessionId, programmeGroupId,showOnlyExtraClasses) => {
    //alert("Yoo");
    let data = new FormData();
    data.append("academicsSessionId", academicsSessionId);
    data.append("programmeGroupId", programmeGroupId);
    data.append("onlyExtraClasses", showOnlyExtraClasses);
    this.setState({ isLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C231CommonAcademicsTimeTableBySessionIdAndProgrammeGroupId`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
      ...
      .then(
        (json) => {
          if (json.CODE === 1) {
            ...
            //   if(rowData.approvalStatusId==1){
            //    colorForApprove="green"
            //   }else if(rowData.approvalStatusId==2){
            //    colorForDecline="red"
            //   }
            //   json.DATA[i].action = (
            //     <Fragment>
            //       <IconButton style={{color:colorForApprove}} disabled={rowData.approvalStatusId!=0} onClick={() => this.UpdateStatus(rowData.id,1)}>
            //       <ThumbUpIcon fontSize="small" />
            //       </IconButton>
            
            //       <IconButton style={{color:colorForDecline}} disabled={rowData.approvalStatusId!=0} onClick={() => this.UpdateStatus(rowData.id,2)}>
            ...
            //   );
            // }
            this.setState({ extraClassesListArray: json.DATA || [] });
          } else {
            this.handleOpenSnackbar(<span>{json.SYSTEM_MESSAGE}<br/>{json.USER_MESSAGE}</span>,"error");
          }
  ...
  onHandleChange = (e) => {
    ...
    switch (name) {
      ...
      case "programmeGroupId":
        ...
        this.loadCourses(this.state.academicSessionId, value,this.state.showOnlyExtraClasses);
        break;
      case "showOnlyExtraClasses":
        this.loadCourses(this.state.academicSessionId, this.state.programmeGroupId,value);
        break;
  ...
  UpdateStatus = async (id,statusId) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C230CommonAcademicsExtraClassTimeTableUpdateStatus`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
      ...
      .then(
        (json) => {
          if (json.CODE === 1) {
            // this.setState({ statusMenuItems: json.DATA });
            this.loadCourses(this.state.academicSessionId,this.state.programmeGroupId,this.state.showOnlyExtraClasses);
          } else {
            this.handleOpenSnackbar(<span>{json.SYSTEM_MESSAGE}<br/>{json.USER_MESSAGE}</span>,"error");
          }
          console.log("statusMenuItems", json);
        },
        (error) => {
          if (error.status == 401) {
            this.setState({
        ...
        <form id="myForm" onSubmit={this.isFormValid}>
          ...
          <Grid 
            ...
            >
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  id="showOnlyExtraClasses"
                  name="showOnlyExtraClasses"
                  variant="outlined"
                  label="Classes"
                  value={this.state.showOnlyExtraClasses}
                  onChange={this.onHandleChange}
                  // error={!!this.state.showOnlyExtraClassesError}
                  // helperText={this.state.programmeGroupIdError}
                  // disabled={!this.state.academicSessionId}
                  // required
                  fullWidth 
          ...
          <Typography
            ...
            >
              ...
              Note* : New schedule added today, will show impact after 12 A.M. 

            </Typography>
              {this.state.extraClassesListArray || !this.state.isLoading ? 
                <F231FormTableComponent
                  rows={this.state.extraClassesListArray}
...
export default withStyles(styles)(F231Form);

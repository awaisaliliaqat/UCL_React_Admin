...
import { 
  Divider, CircularProgress, Grid, Button, Typography, TextField, MenuItem, 
  Dialog, DialogActions, DialogContent, DialogTitle, Table, TableHead, TableBody, TableRow, TableCell, 
  ButtonGroup
} from "@material-ui/core";
import F349FormTableComponent from "./chunks/F349FormTableComponent";
...
class F349Form extends Component {
  constructor(props) {
		super(props);
		this.state = {
			isLoading: false,
	...
	handleCloseDialog = () => {
  	...
  	getYearsData = async (value) => {
		...
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/payroll/C349CommonMonthsView`;
		await fetch(url, {
			method: "POST",
			body: formData,
			headers: new Headers({
	...
	onSearchClick = async (e) => {
		...
		this.setState({ isLoading: true });
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/payroll/C349CommonEmployeePayrollAttendanceCoordinatorView?fromDate=${this.state.fromDateToSend}&toDate=${this.state.toDateToSend}&sessionId=${this.state.academicSessionId}&monthId=${this.state.monthId.id}&isReportingTo=0`;
		await fetch(url, {
			method: "POST",
			headers: new Headers({
				Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),
			...
			(json) => {
				...
				if (CODE === 1) {
					...
					this.setState({
						...
						isExisted: DATA[0].isExist,
						attendanceSheetId: DATA[0].attendanceSheetId !== ""	? DATA[0].attendanceSheetId	: 0,
					});
				} else {
					this.handleOpenSnackbar(<span>{SYSTEM_MESSAGE}<br />{USER_MESSAGE}</span>, "error" );
				}
	...
	onApproveClick = async (e) => {
		...
		this.setState({ isLoading: true });
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/payroll/C349CommonEmployeePayrollAttendanceCoordinatorSendToApproval?attendanceSheetId=${this.state.attendanceSheetId}`;
		await fetch(url, {
			method: "POST",
			headers: new Headers({
				Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),
	...
	handleInputChange = (fieldName, value, rowData) => {
		...
		this.setState({ teachersAttendanceSheetData: updatedData });
	};
	...
	const columns = [
	  	...
	  	{ name: "displayName", title: "Name",
			...
			return (
				<div style={{ whiteSpace: "pre-line" }}>{`${rowData.displayName}`}</div>
			);
			},
			...
			getCellValue: (rowData) => {
				return (
					<TextField
						...
						onChange={(event) =>
							this.handleInputChange("adjustedLateDays",	event.target.value,	rowData)
						}
					/>
				);
			},
			...
			getCellValue: (rowData) => {
				return (
					<TextField
						...
						onChange={(event) =>
							this.handleInputChange( "adjustedAbsentDays", event.target.value, rowData )
						}
					/>
				);
			},
		...
		{ name: "adjustedOverTime", title: "Adjusted Over Time (Hours)",
			getCellValue: (rowData) => {
				return (
					<TextField
						variant="outlined"
						size="small"
						name="adjustedOverTime"
						type="number"
						value={rowData.adjustedOverTime || ""}
						onChange={(event) =>
							this.handleInputChange("adjustedOverTime", event.target.value, rowData)
						}
					/>
				);
			},
			...
			getCellValue: (rowData) => {
				...
				return (
					...
					{attendanceDates.map((item, index) => (
						<div key={`attendanceDates-${index}`}>
						{splitStringByLength(item, chunkSize).map(
							(chunk, chunkIndex) => (
							<div key={`attendanceDates-chunk-${chunkIndex}`}>
								{chunk}
								<br />
							</div>
							)
	  	...
	  	{ name: "totalLateDays", title: "Late Days",
			getCellValue: (rowData) => {
		...
		{ name: "sumLateTime", title: "Late Time",
			getCellValue: (rowData) => {
				const obj = rowData.lateTimeDetails;
				return (
					<div>
	  	...
	  	{ name: "sumEarlyDeparture", title: "Early Departure",
			getCellValue: (rowData) => {
				const obj = rowData.lateTimeDetails;
				return (
					<div>
	  	...
	  	{ name: "sumBreakTime",	title: "Break Time",
			getCellValue: (rowData) => {
				const obj = rowData.lateTimeDetails;
				return (
					<div>
	  	...
	  	{ name: "sumOverTime", title: "Over Time",
			getCellValue: (rowData) => {
				const obj = rowData.lateTimeDetails;
				return (
					<div>
	  	...
	  	{ name: "sumShortTime",	title: "Short Time",
			getCellValue: (rowData) => {
				const obj = rowData.lateTimeDetails;
				return (
					<div>
		...
		<div className={classes.mainContainer}>
		  <div className={classes.titleContainer}>
			<Typography className={classes.title} variant="h5">
				Monthly Employee Deductions For Coordinator
			</Typography>
		  </div>
		  ...
		  <Grid container justifyContent="flex-start" alignItems="flex-start" spacing={2}>
			...
			<Grid item xs={12} md={2}>
				<ButtonGroup variant="text" color="primary" aria-label="text primary button group" fullWidth size="large">
					<Button
						variant="contained"
						color="primary"
		  ...
		  <BottomBar
			...
			disableLeftButton={this.state.isExisted===1 || !this.state?.teachersAttendanceSheetData.length}
			disableRightButton={ this?.state?.isExisted === 1 || !this?.state?.attendanceSheetId }
				  ...
				  <TableBody>
					{Array.isArray(this?.state?.selectedStudent) &&
					  this.state.selectedStudent.map((row, index) => (
						<TableRow key={"selectedStudent-"+index}>
...
F349Form.defaultProps = {
  classes: {},
  setDrawerOpen: (fn) => fn,
};
export default withRouter(withStyles(styles)(F349Form));

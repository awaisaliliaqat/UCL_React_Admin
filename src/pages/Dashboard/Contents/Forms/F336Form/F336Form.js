...
import { Divider, CircularProgress, Grid, Button, Typography, TextField, MenuItem, Dialog, DialogActions, DialogContent, DialogTitle, Table, TableHead, TableBody, TableRow, TableCell, ButtonGroup, } from "@material-ui/core";
import F336FormTableComponent from "./chunks/F336FormTableComponent";
...
class F336Form extends Component {
	constructor(props) {
		super(props);
		this.state = {
			isLoading: false,
							...
							{attendanceDates.map((item, index) => (
								<div key={`attendanceDates-${index}`}>
									{splitStringByLength(item, chunkSize).map(
										(chunk, chunkIndex) => (<div key={`attendanceDates-chunk-${chunkIndex}`}>{chunk}<br /></div>))
									}
								</div>
							))}
						</div>
						...
						<Grid item xs={12} sm={6} md={3}>
							...
							>
								{this.state.yearData?.map((item, index) => (
									<MenuItem key={"yearData-"+index} value={item}>
										{item.monthName}
									</MenuItem>
								))}
							</TextField>
...
F336Form.defaultProps = {
	classes: {},
	setDrawerOpen: (fn) => fn,
};

export default withRouter(withStyles(styles)(F336Form));

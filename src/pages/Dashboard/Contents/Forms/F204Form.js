...
class F204Form extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getCoursesData = async() => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/lms/C204CommonAcademicsCoursesView`;
    await fetch(url, {
      method: "POST",
      headers: new Headers({
      ...
      .then(
        (json) => {
          ...
          console.log("getCoursesData", json);
        },
        (error) => {
          if (error.status == 401) {
            this.setState({
  ...
  getSectionsData = async(courseId) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/lms/C204CommonAcademicsSectionsView`;
    await fetch(url, {
      method: "POST",
      body:data,
      headers: new Headers({
      ...
      .then(
        (json) => {
          ...
          console.log("getSectionsData", json);
        },
        (error) => {
          if (error.status == 401) {
            this.setState({
      ...
      .then(
        (json) => {
          if (json.CODE === 1) {
            ...
            if(dataLength){
              this.getSectionsData(data[0].courseId);
              this.setState({
                courseId:data[0].courseId,
                sectionId:data[0].sectionId,
                label: data[0].label,
                startDate: data[0].startDate,
                startTime: data[0].startTimeReport,
                endDate: data[0].endDate,
                endTime: data[0].endTimeReport,
                totalMarks: data[0].totalMarks,
                instruction: data[0].instruction,
              });
            }else{
              window.location = "#/dashboard/F204Form/0";
            }
          } else {
            this.handleOpenSnackbar(<span>{json.SYSTEM_MESSAGE}<br/>{json.USER_MESSAGE}</span>,"error");
          }
  ...
  isLabelValid = () => {
    let isValid = true;
    if (!this.state.label) {
      this.setState({ labelError: "Please enter exam label." });
  ...
  onFormSubmit = async () => {
    ...
    this.setState({ isLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/lms/C204CommonAcademicsExamsSave`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
          ...
          if (json.CODE === 1) {
            ...
            setTimeout(() => {
              if (this.state.recordId != 0) {
                window.location = "#/dashboard/F204Reports";
              } else {
...
export default withStyles(styles)(F204Form);
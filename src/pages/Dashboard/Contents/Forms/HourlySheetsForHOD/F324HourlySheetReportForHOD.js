...
import F322HourlySheetReportFilterForCoordinator from "./F324HourlySheetReportFilterForHOD";
import F322HourlySheetReportTableComponentForCoordinator from "./F324HourlySheetReportTableComponentForHOD";
...
class F324HourlySheetsForHOD extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  getData = async (status) => {
    ...
    const eventDataQuery = this.state.eventDate
      ...
      : "";
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C324TeachersProgrammeGroupAttandanceAprrovalView`;
    await fetch(url, {
      method: "GET",
      headers: new Headers({
        Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),
            ...
            for (var i = 0; i < json.DATA.length; i++) {
              json.DATA[i].action = (
                <ViewTableRecord
                  ...
                  onEditURL={`/dashboard/F324ViewRecordData/${json.DATA[i].id}`}
...
export default F324HourlySheetsForHOD;

...
import F361ReportsTableComponent from "./Chunks/F361ReportsTableComponent";
...
class F361Reports extends Component {
	constructor(props) {
		super(props);
		this.state = {
			isLoading: false,
	...
	getData = async () => {
		...
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/payroll/C361CommonEmployeesPayrollRequests/View`;
		await fetch(url, {
			method: "POST",
			body: data,
			headers: new Headers({
					...
					<Grid item xs={12}>
						...
						>
							<Tooltip title="Back">
								<IconButton
									onClick={() =>
										window.location.replace("#/dashboard/F361Form")
									}
								>
...
export default withStyles(styles)(F361Reports);

...
import F202ReportsTableComponent from "./F202ReportsTableComponent";
...
class F202Reports extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  getData = async () => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C202CommonAcademicsSessionsSeminarEvaluationsView`;
    await fetch(url, {
      method: "POST",
      headers: new Headers({
            ...
            for (var i = 0; i < json.DATA.length; i++) {
              ...
              json.DATA[i].action = (
                <EditDeleteTableComponent
                  ...
                  editRecord = {()=>window.location.replace(`#/dashboard/F202Form/${id}`)}

...
class R307ApprovedProgressReportsFilter extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  loadAcademicSession = async () => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C307CommonAcademicSessionsView`;
    await fetch(url, {
      method: "POST",
      headers: new Headers({
  ...
  loadTerms = async (academicsSessionId) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C307CommonAcademicsSessionsTermsView`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
  ...
  loadProgrammes = async (academicSessionId) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C307CommonAcademicsSessionsOfferedProgrammesView`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
  ...
  handleGenerate = () => {
    ...
    window.open(`#/R307ApprovedStudentProgressReports/${academicSessionId+"&"+programmeId+"&"+sessionTermId+"&"+this.state.endDate}`,"_blank");
  };
          ...
          >
            <Grid item xs={12}>
              <Typography
                ...
                variant="h5"
              >
                Approved Students Progress Report
              </Typography>
              <Divider
                style={{
            ...
            <Grid item xs={12} md={4}>
              <TextField
                ...
                select
              >
                {this.state.academicSessionIdMenuItems.map((dt) => (
                  <MenuItem
            ...
            <Grid item xs={12} md={4}>
              ...
              >
                {this.state.programmeIdMenuItems ? (
                  this.state.programmeIdMenuItems.map((dt) => (
                    <MenuItem
            ...
            <Grid item xs={12} md={4}>
              <TextField
                ...
                select
              >
                {this.state.sessionTermMenuItems.map((dt) => (
                  <MenuItem
...
R307ApprovedProgressReportsFilter.propTypes = {
  setDrawerOpen: PropTypes.func,
  isDrawerOpen: PropTypes.bool
};

R307ApprovedProgressReportsFilter.defaultProps = {
  setDrawerOpen: fn => fn,
  isDrawerOpen: false,
  
};

export default withStyles(styles)(R307ApprovedProgressReportsFilter);
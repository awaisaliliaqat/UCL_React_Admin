...
import ChangeStudentProgrammeFilter from "./Chunks/ChangeStudentProgrammeFilter";
...
class ChangeStudentProgramme extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
...
onHandleChangeStatus = e => {
  ...
  getData = async () => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C222CommonStudentsView?programmeGroupId=${this.state.programmeGroupId||0}&academicSessionId=${this.state.academicSessionId||0}&programmeId=${this.state.programmeId||0}`;
    await fetch(url, {
      method: "GET",
      headers: new Headers({
        Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),
      ...
      .then(
        (json) => {
          if (json.CODE === 1) {
            this.setState({ studentsData: json.DATA || [] });
...
export default ChangeStudentProgramme;

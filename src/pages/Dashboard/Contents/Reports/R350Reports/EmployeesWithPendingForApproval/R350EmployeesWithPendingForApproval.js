...
class R350EmployeesWithPendingForApproval extends Component {
	constructor(props) {
		super(props);
		this.state = {
			isLoading: false,
	...
	getAcademicSessions = async (academicId, monthId) => {
		...
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/payroll/C350EmployeesPendingApprovalsView?sessionId=${academicId}&sessionPayrollMonthId=${monthId}`;
		await fetch(url, {
			method: "POST",
			headers: new Headers({
	...
	render() {
		...
		const columns = [
			...
			{ name: "createdBy", title: "Submitted By"},
			{ name: "submittedTo", title: "Submitted To" },
...
R350EmployeesWithPendingForApproval.propTypes = {
	classes: PropTypes.object,
	setDrawerOpen: PropTypes.func,
};

R350EmployeesWithPendingForApproval.defaultProps = {
	classes: {},
	setDrawerOpen: (fn) => fn,
};
export default withRouter(
	withStyles(styles)(R350EmployeesWithPendingForApproval)
);

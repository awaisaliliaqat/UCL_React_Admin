...
import R348TimeTableActivationViewTableComponent from "./Chunks/R348TimeTableActivationViewTableComponent";
import { withRouter } from "react-router-dom";
import CloudDownloadIcon from "@material-ui/icons/CloudDownload";

class R348TimeTableActivationView extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  getData = async () => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C348CommonAcademicsTimeTableProgrammeGroupStatusView`;
    await fetch(url, {
      method: "GET",
      headers: new Headers({
  ...
  DeleteData = async (id) => {
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C348CommonAcademicsTimeTableProgramGroupStatusDelete?programmeGroupStatusId=${id}`;
    await fetch(url, {
      method: "DELETE",
      // body: data,
      headers: new Headers({
  ...
  render() {
    const columns = [
      { name: "programmeGroupId", title: "Program Group ID" },
      { name: "programmeGroupLabel", title: "Programme Group Label" },
      // { name: "programmeGroupId", title: "Program Group ID" },

      { name: "pauseFrom", title: "Pause From" },
      { name: "pauseTo", title: "Pause To" },
      {
        name: "Action",
        title: "Action",
        getCellValue: (rowData) => {
          return (
            <Button
              onClick={() => this.DeleteData(rowData.programmeGroupStatusId)}
              style={{
    ...
    return (
      <Fragment>
        ...
        >
            ...
            >
              <Tooltip title="Back">
                <IconButton
                  onClick={() =>
                    window.location.replace(
                      "#/dashboard/F348DefineTimetableActivation/0"
                    )
                  }
                >
              ...
              Timetable Pause/Unpause Report
            </Typography>
          ...
          />
          <R348TimeTableActivationViewTableComponent
...
export default withRouter(R348TimeTableActivationView);

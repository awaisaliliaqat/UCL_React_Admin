...
class R341Reports extends Component {
  ...
  getData = async () => {
    ...
    }/lms/C341CommonPendingStudentsFeedbackView?academicSessionId=${
      this.state.academicSessionId
    }&termId=${this.state.termId}&programmeGroupId=${
      this.state.programmeGroupId
    }&programmeId=${this.state.programId}&courseId=${
      this.state.courseId ? this.state.courseId.id : ""
    }&filter=${this.state.schoolId}`;
    await fetch(url, {
      method: "GET",
      headers: new Headers({
        Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),

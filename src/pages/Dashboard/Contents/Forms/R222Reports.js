...
class R222Reports extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  loadSection = async (academicSessionId) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C218CommonAcademicsSectionsView`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
  ...
  handleGenerate = () => {
    ...
    window.open(`#/R222StudentProgressReport/${academicSessionId+"&"+sectionId+"&"+sessionTermId}`,"_blank");
  };
...
export default withStyles(styles)(R222Reports);
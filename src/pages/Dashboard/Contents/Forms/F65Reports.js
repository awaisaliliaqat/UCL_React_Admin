...
import F65ReportsTableComponent from "./F65ReportsTableComponent";
...
class AnnouncementReports extends Component {
            ...
            for (var i = 0; i < json.DATA.length; i++) {
              ...
              json.DATA[i].action = (
                <EditDeleteTableComponent
                  ...
                  editRecord={() => window.location.replace(`#/dashboard/F65Form/${id}`)}
                />
              );
  ...
  DeleteData = async (event) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/lms/C65CommonAcademicsTeacherAnouncementsDelete`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
      ...
      <Fragment>
        ...
        >
          ...
          >
            ...
            >
              <Tooltip title="Back">
                <IconButton onClick={() => window.location.replace('#/dashboard/F65Form/0')}>

...
import DefineEmployeeEntitiesTableComponent from "./chunks/DefineEmployeeSubDepartmentsTableComponent";
...
class F311DefineEmployeeSubDepartmentsFrom extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      employeesDepartmentsTypesData: [],
      employeesDepartmentsTypesDataLoading: false,
      employeesDepartmentsObject: {},
      employeesDepartmentsObjectError: "",
  ...
  getEmployeeRolesTypesData = async (id) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C311CommonEmployeesRolesTypesView`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
  ...
  getEmployeesEntitiesData = async (roleObject) => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C311CommonEmployeesEntitiesTypesView`;
  ...
  getEmployeesDepartmentsData = async (entityObject) => {
    this.setState({ employeesDepartmentsTypesDataLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C311CommonEmployeesEntitiesDepartmentsTypesView`;
    let data = new FormData();
    ...
    await fetch(url, {
      ...
      .then(
        (json) => {
          if (json.CODE === 1) {
            this.setState({
              employeesDepartmentsTypesData: json.DATA || [],
            });
          } else {
            this.handleOpenSnackbar(
              json.USER_MESSAGE + "\n" + json.SYSTEM_MESSAGE,
    ...
    this.setState({ employeesDepartmentsTypesDataLoading: false });
  };

  getEmployeesSubDepartmentsData = async (departmentObject) => {
    this.setState({ isLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C311CommonEmployeesEntitiesSubDepartmentsTypesView`;
    let data = new FormData();
    data.append("departmentId", departmentObject.id);
    await fetch(url, {
      method: "POST",
      headers: new Headers({
        Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),
  ...
  onHandleChange = (e) => {
    ...
    switch (name) {
      case "employeesRolesObject":
        ...
        break;
        case "employeesDepartmentsObject":
            if(value?.id){
              this.getEmployeesSubDepartmentsData(value || []);
            } else {
              this.setState({
                employeesSubDepartmentsData: [],
              })
  ...
  onFormSubmit = async (e) => {
    ...
    if (this.isFormValid) {
      ...
      data.append("departmentTypeId", this.state.employeesDepartmentsObject.id);
      this.setState({ isLoading: true });
      const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C311CommonEmployeesEntitiesSubDepartmentsTypesSave`;
      await fetch(url, {
        method: "POST",
        body: data,
        headers: new Headers({
        ...
        .then(
          (json) => {
            if (json.CODE === 1) {
              //alert(json.USER_MESSAGE);
              this.getEmployeesSubDepartmentsData(this.state.employeesDepartmentsObject);
  ...
  onDeleteRecord = async (e, id) => {
    ...
    data.append("id", id);
    this.setState({ isLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C311CommonEmployeesEntitiesSubDepartmentsTypesDeleteById`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
        ...
        <form id="myForm" onSubmit={this.onFormSubmit}>
          ...
          <Grid container component="main" className={classes.root}>
            <Typography className={classes.pageTitle} variant="h5">
              Define Employee Sub Departments
            </Typography>
            ...
            <Grid container spacing={2} className={classes.container}>
              ...
              <Grid item xs={6}>
                <Autocomplete
                  id="employeesDepartmentsObject"
                  getOptionLabel={(option) =>
                    typeof option.label === "string" ? option.label : ""
                  }
                  fullWidth
                  aria-autocomplete="none"
                  options={this.state.employeesDepartmentsTypesData}
                  loading={this.state.employeesDepartmentsTypesDataLoading}
                  value={this.state.employeesDepartmentsObject}
                  onChange={(e, value) =>
                    this.onHandleChange({
                      target: { name: "employeesDepartmentsObject", value },
                    })
                  }
                  ...
                  renderInput={(params) => {
                    ...
                    return (
                      <TextField
                        variant="outlined"
                        error={!!this.state.employeesDepartmentsObjectError}
                        helperText={this.state.employeesDepartmentsObjectError}
                        inputProps={inputProps}
                        label="Departments *"
                        {...params}
                      />
            ...
            <Grid container spacing={2} className={classes.reportsContainer}>
              <Grid item xs={12}>
                ...
                <DefineEmployeeEntitiesTableComponent
                  rows={this.state.employeesSubDepartmentsData || []}
                  columns={columns}
...
F311DefineEmployeeSubDepartmentsFrom.propTypes = {
...
F311DefineEmployeeSubDepartmentsFrom.defaultProps = {
  isDrawerOpen: true,
  setDrawerOpen: (fn) => fn,
  match: {
    params: {
...
export default withStyles(styles)(F311DefineEmployeeSubDepartmentsFrom);

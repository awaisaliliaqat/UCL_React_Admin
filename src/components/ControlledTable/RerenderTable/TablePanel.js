...
const TablePanel = ({ classes, columns, data, onClick, sortingEnabled, isLoading, isShowIndexColumn, isDense }) => {
  ...
  React.useEffect(() => {
    ...
    if (sortedData.length > 0) {
      ...
      if (diff.length !== 0) {
        const sort = sortDirection === 'asc' ? ascSort(data, index) : descSort(data, index);
        setSortedData(sort);
        setPerData(data);
      }
    }
  ...
  return (
    <Fragment>
      ...
      <Table size={isDense ? 'small' : 'medium'} className={classes.table}>
        ...
        />
        <TableContent isShowIndexColumn={isShowIndexColumn} onRowClick={rowData => onClick(rowData)} columns={columns} data={tableData} />
      </Table>
    </Fragment>
  );
};

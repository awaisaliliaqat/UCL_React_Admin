...
class F346StudentAttendanceFeature extends Component {
	...
	loadAcademicSessions = async () => {
		...
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C346CommonAcademicSessionsView`;
		await fetch(url, {
			method: "POST",
			headers: new Headers({
	...
	getSchools = async () => {
		this.setState({ isLoading: true });
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C346CommonSchoolsView`;
		await fetch(url, {
			method: "POST",
			headers: new Headers({
				Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),
	...
	getProgramGroup = async (schoolId) => {
		...
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C346CommonProgrammeGroupsView`;
		await fetch(url, {
			method: "POST",
			body: data,
			headers: new Headers({
	...
	getProgrammes = async (sessionId, programmeGroupId) => {
		if (sessionId && programmeGroupId) {
			...
			const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C346CommonProgrammesView`;
			await fetch(url, {
				method: "POST",
				body: data,
				headers: new Headers({
	...
	getCourse = async (sessionId, programmeGroupId) => {
		if (sessionId && programmeGroupId) {
			...
			data.append("programmeGroupId", programmeGroupId);
			data.append("sessionId", sessionId);
			const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C346CommonProgrammeCoursesView`;
			await fetch(url, {
				method: "POST",
				body: data,
				headers: new Headers({
	...
	getData = async () => {
		this.setState({ isLoading: true });
		const formattedDate = this.state.fromDate.toLocaleDateString("en-US", {year: "numeric",	month: "long"});
		const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/payroll/C346StudentAttendanceFeature?academicSessionId=${ this.state.academicSessionId }&schoolId=${this.state.schoolId}&programmeGroupId=${ this.state.programmeGroupId }&programmeId=${this.state.programmeId}&courseId=${ this?.state?.courseId ? this.state.courseId.id : 0 }&month=${formattedDate}`;
		await fetch(url, {
			method: "GET",
			headers: new Headers({
				Authorization: "Bearer " + localStorage.getItem("uclAdminToken"),

...
class UploadTutionFeesAction extends Component {
  ...
  render() {
    ...
    const findID = academicSessionMenuItems.find(
      (item) => item.ID === sessionId
    );

    console.log(findID, "id is coming");

    const handleNavigate = () => {
      if (findID !== undefined) {
        window.location = `#/dashboard/R327Uploaded-Fee-Voucher/${findID.Label}T${findID.ID}`;
      }
    };
    ...
    const columns = [
      {
        ...
        dataIndex: "id",
        sortable: false,
        customStyleHeader: { width: "8%" },
      },
      {
        name: "File",
        dataIndex: "fileName",
        sortable: false,
        customStyleHeader: { width: "30%" },
      },
      {
        name: "Session ID",
        dataIndex: "academicSessionLabel",
        sortable: false,
          ...
          return (
            <Fragment>
              <div style={{ display: "flex", margin: "-10px" }}>
                <IconButton
                  onClick={() => downloadFile(rowData.fileName)}
                  aria-label="download"
                >
                  <CloudDownloadIcon />
                </IconButton>
      ...
      <Fragment>
        <form autoComplete="off" onSubmit={handleSubmit}>
          ...
          >
            ...
            >
              {" "}
              {values.uploadLoading || values.isLoading ? (

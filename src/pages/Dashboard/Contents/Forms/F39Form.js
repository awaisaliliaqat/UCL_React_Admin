...
function CourseRow(props) {
  ...
  const [prerequisiteCourse, setPrerequisiteCourse] = useState(rowData.courseSubjectExclusionsSelected);
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <TextField
          type="hidden"
          name="programmeCourseIdSubjectExclusions"
...
class F39Form extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  loadProgrammeGroups = async () => {
    ...
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C39CommonProgrammeGroupsView`;
    await fetch(url, {
      method: "POST",
      headers: new Headers({
  ...
  loadProgrammeGroupCourses = async (programmeGroupId) => {
    ...
    this.setState({ isLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C39CommonProgrammeCoursesView`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
          ...
          if (json.CODE === 1) {
            ...
            for (let i = 0; i < json.DATA.length; i++) {
              let obj = {
                ...
                Label:json.DATA[i].displayLabel,
              };
  ...
  loadData = async (index) => {
    ...
    data.append("programmeGroupId", index);
    this.setState({ isLoading: true });
  ...
  onFormSubmit = async (e) => {
    ...
    this.setState({ isLoading: true });
    const url = `${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_SUB_API_NAME}/common/C39CommonCourseSubjectExclusionsSave`;
    await fetch(url, {
      method: "POST",
      body: data,
      headers: new Headers({
          ...
          if (json.CODE === 1) {
            ...
            setTimeout(() => {
              if (this.state.recordId != 0) {
                window.location = "#/dashboard/F39Reports";
              } else {
...
export default withStyles(styles)(F39Form);

...
class F31Form extends Component {
	...
	getPreTimeSlotsMenuItems = () => {
		...
		const startTime = new Date(); // any date is fine; we only use time
		startTime.setHours(8, 0, 0, 0); // 08:00 AM

		const endTime = new Date(startTime);
		endTime.setHours(20, 0, 0, 0); // 08:00 PM

		const times = [];
		let current = new Date(startTime);

		while (isBefore(current, addMinutes(endTime, interval))) {
			const timeString = format(current, 'hh:mm a'); // e.g., "08:00 AM"
